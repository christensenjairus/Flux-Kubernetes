...
helm install connect 1password/connect --namespace onepassword-connect --create-namespace --set-file connect.credentials=/tmp/1password-credentials.json --set operator.create=true --set operator.token.value=$OP_CONNECT_TOKEN #--set connect.nodeSelector.nodeclass=general --set operator.nodeSelector.nodeclass=general
